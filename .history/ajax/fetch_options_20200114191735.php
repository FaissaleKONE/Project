<?php
    ...
    $connect = new PDO("mysql:host=$server;dbname=", $login, $pass);
        ...
        $error = 0;
            ...
            if($error == 0){
                    $query = "
                    SELECT * FROM client WHERE client_phone = '".$phone."'
                    ";
                    ...
                    if($total_row > 0){
                        ...
                        $query = "
                        insert into client(student_first_name,student_last_name,student_phone,student_sexe,student_situation,parent_phone1,parent_phone2,student_email_parent) values(?,?,?,?,?,?,?,?)
                        ";
                        $statement = $connect->prepare($query);
